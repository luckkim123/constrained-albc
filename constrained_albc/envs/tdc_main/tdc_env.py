...
`main.ALBCEnv` so the baseline faces identical conditions to the RL policy.
...
own docstring). The thruster PD controller still drives all 6 wrench
components (Fx,Fy,Fz,Tx,Ty,Tz) for a fair comparison against the RL policy's
full thruster authority, so `lin_vel_cmd_body` is fed a zero target: with no
...
class ALBCTDCEnv(ALBCEnv):
    """Attitude-only ALBC environment driven by classical controllers (no RL)."""
    ...
    def _compute_classical_actions(self) -> torch.Tensor:
        ...
        # access to the full thruster authority. `main` is attitude-only and
        # has no linear-velocity command, so the lin-vel target is zero
        # (station-keep) rather than a policy-commanded `_vel_cmd_lin`.
        thruster_cmds = self._thruster_pd.compute(
            lin_vel_cmd_body=torch.zeros_like(lin_vel_b),
            roll=roll,
            pitch=pitch,
