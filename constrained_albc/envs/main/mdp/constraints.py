...
config -- wired in by the experiment, reward centering removed 2026-07):
    joint1_cumulative  (avg)   |q_des_1 - nominal|       (integrated command)
"""
...
def joint1_cumulative_cost(
    ...
    """|integrated joint1 command - nominal| (joint1 anti-drift constraint).
    ...
    albc_env._apply_joint_pd_action) rather than a measured wrapped angle.
    _joint_pos_targets is already an UNWRAPPED running sum (never folded to
    (-pi, pi]), so a full revolution of drift is a real accumulated displacement,
    not folded to zero -- which a wrapped instantaneous form (the removed arm A /
    reward centering) could not see. Displacement from nominal in either direction
    counts (drift is one-directional, but either sign). No extra env accumulator is
    needed: the integrator already holds the unwrapped sum, and it is reset to the
    measured pose on episode reset (albc_env._reset_action_buffers).

    This is the sole joint1 anti-drift mechanism: the reward-side centering penalty
    and the wrapped-instantaneous constraint (arm A) were both removed 2026-07.
    """
...
def apply_joint1_constraint_arm(env_cfg) -> None:
    ...
    silently leave the shipped 10-term set untouched (a baseline-dup masquerading as arm B).
    ...
    if arm == "none":
        return
    if arm != "B":
        raise ValueError(f"joint1_constraint_arm must be one of 'none'/'B', got {arm!r}")
