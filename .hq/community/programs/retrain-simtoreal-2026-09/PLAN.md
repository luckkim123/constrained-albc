...
**Status: REVISION 3.2 (2026-09-03) — 3.1 (rebuilt from primary sources, codex ground-4 BLOCK review folded in, §12) plus: literature R1–R5 re-verified against primary text (`finding/308`), thrust-coefficient correction and the m0-probe vertical-moment arithmetic (`finding/309`). NOT approved, NOT launched, nothing queued.**
...
| 2 | Vertical Fz/My "single-motor" redesign | premise REFUTED (two motors, `finding/255` correction 08-13); placement ±x correct; **magnitude: lever verified, but the m0 probe puts the effective moment at u = 0.25 at ≈ 0.31× the linear plant's 0.145 m × 40 N/unit — coefficient error vs quadratic law undecided** (`finding/309`) | post-item-2 on topology; **pre-R-1** (R-1's two-level ratio decides shape vs coefficient) |
...
| vertical TAM `My` row / thrust model | `±0.145` (geometry from `actuators.xacro`, reproduces to 4 decimals); thrust = command × 40 N **linear**, `thrust_coefficient_scale (0.7, 1.3)`, clamp 50 N × `(0.85, 1.15)`, `enable_thrust_curve false` | **held at 0.145, linear plant held** for this revision. The m0 probe puts the effective moment at u = 0.25 at ≈ 0.455 N·m vs 1.45 N·m linear nominal (0.31×, below the ±30 % coefficient-DR floor) and vs 0.363 N·m for the sim's own signed-square curve (1.25×, inside its band) — one level cannot separate a 3× coefficient error from a quadratic law. `[DECISION-REQUIRED: vertical-moment]` now has four options: (a) hold, (b) scale `thrust_coefficient`, (c) screen a band in S1c, **(d) `enable_thrust_curve true` with `thrust_deadband 0`** (all six channels; the mixer already inverts the ESC deadband, `finding/281`). R-1's ratio readout selects between (b) and (d); nothing changes in Phase 3 without the decision | `finding/309` (arithmetic on `deployed_tam.json` m0 basis + `vault:finding/136` K); `finding/283` (2× at full command, ≈ 4.65× in the policy's band, Stonefish vs Isaac — the same shape question); `finding/305`: placement correct; codex `review/307`: a measurement must not rewrite the plant unreviewed |
...
|:--|:--|:--|:--|:--|
| G0-A | Does the incumbent have an arm-pitch fallback? | `eval.py static --checkpoint model_9998 --fault_fixed_health 1,1,1,0,0,1 --doraemon-dr-from <incumbent>` with the attitude STEP trajectory (`--att-amp-deg 15`), 64 envs × the standard episode set, seed 42, at `none` AND `hard`; compare to health `1,1,1,1,1,1` | pitch tracking ratio fault/healthy (paired per env, decision floor 0.10° ss_error, `decision/117`): ≥ 0.8 at both levels → fallback exists (D-1 becomes "robustness"); ≤ 0.5 at both → D-1 confirmed; **anything else → inconclusive, report both levels and the θ2 span, no verdict** (thresholds are pre-registered choices, not derived) | how §9 is phrased, not whether the program runs |
| G0-B | Incumbent delay sensitivity | `--control-delay 0/1/2` at all levels, same seed/env set | pre-registered: if att ss_error at d = 1 is ≥ 2× d = 0 at `hard`, delay DR (0,1) is worth its cost; if < 1.3×, `[DECISION-REQUIRED: delay-range]` may drop it | (0,1) go/no-go |
...
| **G0-I** (new) | Sampler exposure unit test | implement §5 (1)(2) behind two config values (the mechanism can be built before the values are decided; the VALUES wait for `fault-config`); test asserts the closed-form numbers ± 0.5 pp at fixed severity and the severity-integrated numbers with s ~ U(0,1), and all-healthy at severity 0 | test green | gates G0-C |
| **G0-J** (new) | Launch manifest | pin the commit; review `git diff 598db899..<SHA> -- constrained_albc/envs/main/`; dirty tree empty or listed; Hydra config diff vs `deployed_env.yaml` equals the §5 delta and nothing else; record SHA + config hash in the run group README | diff reviewed, hashes recorded | gates G0-C and Phase 3 |
...
|:--|:--|:--|:--|
| **R-1** | Vertical pitch-moment **identification** (effective N·m per unit command — thrust coefficient × lever, NOT the lever alone; the geometry is already verified by xacro) | Robot free-floating mid-tank, no contact, no tether load; attitude controllers OFF; all other thrusters 0; arm parked at θ2 = 150°, J1 fixed for the whole run (its constant moment cancels in the differential). Command m0 at levels u ∈ {0, +0.25, +0.5, 0, −0.25, −0.5, 0} through the mixer (`undeadband` on), 15 s each, repeated twice. Readout: **Δθ between adjacent levels** (differential, so the arm/trim offset drops out) via `rotate_imu`; `M(u) = K·Δθ`, K = 7.76 N·m/rad (`vault:finding/136`). Steady-state criterion: tilt rate < 0.5 °/s for the last 5 s; discard a level if depth changes > 0.3 m or a wall is reached before that. Log depth, `thruster_pwm`, gyro. Compare `M(u)/u` to sim `0.145 m × 40 N/unit = 5.8 N·m` (coefficient-DR band 4.1–7.5 N·m). **Decisive readout (3.2): r = Δθ(0.5)/Δθ(0.25) per sign from adjacent-level differences — r ≈ 2 → linear law, coefficient off → option (b); r ≈ 4 → quadratic law → option (d); in between → report both, no automatic pick. K cancels in r, so the shape verdict does not inherit K's n = 8 uncertainty; K enters only the absolute M(u)/u** | HARD-GATE items 2 and 4 — **as an input to `[DECISION-REQUIRED: vertical-moment]`, never an automatic plant edit.** Known limits: cannot separate lever from thrust coefficient; heave coupling bounds the usable dwell; K itself is a static-map estimate (n = 8 points) |
...
`[DECISION-REQUIRED: r1-before-final]` — whether R-1 must precede Phase 3. **Recommendation changed in 3.2: yes.** The m0 probe shows a ≈ 3× gain gap at small commands (`finding/309`), outside the ±30 % coefficient DR, so Phase 3 without R-1 trains on a vertical channel the record already says is wrong in magnitude; R-1 costs ≈ 30 min of tank time. Fallback if the tank slips: Phase 3 runs pre-R-1 as §3 records and S1c screens a band.
...
9. `[DECISION-REQUIRED: vertical-moment]` — what to do with R-1's numbers. Options: (a) hold 0.145 m × 40 N/unit linear; (b) scale `thrust_coefficient` to the measured effective moment (geometry stays — the lever is verified); (c) add a thrust-coefficient DR band on m0/m3 as screening arm S1c; **(d) `enable_thrust_curve true` + `thrust_deadband 0`** — the sim's existing signed-square law on all six channels (`marinelab/core/thruster.py:164-182`), deadband zeroed because the deployment mixer already inverts the ESC deadband (`finding/281`); a plant-shape change beyond the two-knob scope, so it needs its own paired probe. Recommend: let R-1's ratio pick — r ≈ 4 → (d); r ≈ 2 → (b), or (a) if M(0.5)/0.5 is within ±30 % of 5.8 N·m. No option is applied without this decision (`finding/309`).
