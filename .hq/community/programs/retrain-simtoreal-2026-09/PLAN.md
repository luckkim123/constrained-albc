...
**Status: REVISION 3.1 (2026-09-03) — rebuilt from primary sources, then corrected on a codex ground-4 BLOCK review (11 findings; 8 folded in, 2 rejected with reasons, 1 partly — see §12). NOT approved, NOT launched, nothing queued.**
...
| vertical TAM `My` row | `±0.145` (geometry from `actuators.xacro`, reproduces to 4 decimals) | **held at 0.145.** No automatic correction from R-1: R-1 identifies the effective moment per unit command (thrust coefficient × lever, not the lever alone), so a deviation is `[DECISION-REQUIRED: vertical-moment]` — the user chooses between (a) hold, (b) scale the vertical thrust coefficient (geometry stays), (c) screen a band in S1c. Nothing changes in Phase 3 without that decision | `finding/305`: placement correct, magnitude unmeasured; "over-estimated" has no number in the record (`vault:finding/137` §7(2)) — a plan must not encode a direction with no measurement, and must not let a measurement rewrite the plant unreviewed (codex review, `review/307`) |
...
9. `[DECISION-REQUIRED: vertical-moment]` — what to do with R-1's number. Options: (a) hold 0.145 × 50 N (recommended if within ±20 %); (b) scale the vertical thrust coefficient to the measured effective moment (geometry stays — the lever is verified); (c) add a thrust-coefficient DR band on m0/m3 as screening arm S1c (width e.g. ±20 % around the measured value). No option is applied without this decision.
