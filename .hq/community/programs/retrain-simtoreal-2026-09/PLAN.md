...
## Update 2026-09-08 (p5 round closed by the overnight session; operator ended the session ~15:00)

**Chain**: teacher `trpo_p5_s30_260907_131300` (0-4452, lb 200, curriculum dead) -> resumed 17:57 as `trpo_p5_s30_r4450_260907_175721` with `performance_lb` -20 (decision/408) -> DONE 23:49 exit 0, `model_9999.pt`. Milestone exams 2500/5000/7500 were own-sidecar and ranked 5000 first; the operator chose "재채점 후 선발" (22:32) -> `p5_regrade.sh` froze the final sidecar (`g0c_runner/p5_frozen_final/`) and regraded 5000/7500 on it -> **pick = model_9999** (medium paired att 9.33 vs 7500 13.76 vs 5000 18.41; `p5_pick.log`). Own-moment scores kept as `.hq/work/p5/own_5000`, `own_7500`, `pre_2500`. Student `trpo_sd_p5_r3a_s30_260908_015931` (R3a recipe from model_9999) 01:59-02:10, 999 it, beta 0, loss_action 0.0029 / loss_latent 0.075; exam `sd_p5_r3a` beats the teacher on all 7 cells at every DR level (healthy medium 5.34 vs 5.96, healthy_d1 5.34 vs 11.21, healthy_d8 17.0 vs 19.9), survival 100 %. Pack `deploy/retrain_simtoreal_p5/pack_p5_r3a_gru_260908_031727` 03:17, parity CLOSED.

**Gates**: container self-close CLOSED (act 1.2e-7, latent 1.7e-7, hidden 1.3e-7); Mac sha256 5/5; Mac numpy 2.4.6 self-close CLOSED; board package tests on the Mac copy of `agent-jetson/robot/albc_rl` 12 passed / **2 xpassed** (tick-1 saturation + runaway xfails pass). Pack + `board_files/` + zip (sha acfd9720...) + `MACBOOK_DEPLOY_PROMPT.md` at Mac `~/albc_deploy/retrain_simtoreal_p5/`. OPEN: TX2 board gate (jump host offline), field run. NOTE: Mac `01_code/albc_numpy_port/` is a stale snapshot (npforward sha 3c1c250c, normalize differs) -- not the board reference.

**Merge**: cleanup/2026-09 landed after P5_PACK_DONE: albc a3c3081 (.hq records) -> 2472e85 (merge b10d4df) -> 1d90a4e (`merge -s ours main`, operator decision); main fast-forwarded to 1d90a4e; exp + main pushed. marinelab ff to 3f5d4fa (main ahead 8 of origin, NOT pushed -- operator-deferred). Live pytest 517 passed / 9 skipped. `EXPECT_HEAD=1d90a4e` in `p5_common.sh`. Scratch worktrees removed.

**Report**: `experiments/rsl_rl/albc_trpo_teacher/retrain_simtoreal_p5/trpo_p5_s30_r4450_260907_175721/analysis/diagnose-20260908-142536/report.md` (6.6k words, 25 findings, 157 table rows, 3 plots; `omx report-coverage --min-coverage 0.5` ok, engine cited; `omx report-review` approve; wiki capture-session finding/411-435; curated finding/436-440 incl. two `needs-experiment` and one engine-gap). NOT done: `report.ko.md` (Korean mirror) and the independent `report-reviewer` agent pass -- both must go through the exp-analyze RE-analysis path with this report as BASE. Cross-run rows (p4 `inc13w`/`sd_inc9998`/`sd_r3a` on the p3b plant) are cited by summary path, not run through `--cross-run-refs`. `experiments/` is gitignored: the report exists only on the container disk.

**Report headline items**: (1) curriculum dead 4450 it at lb 200, revived by lb -20; all 23 dims still widening at 9999, paced dims at 5-8 % of range -- under-trained on lag/fault/disturbance by construction; (2) own-sidecar milestone ranking is a plant artifact (5000 own 5.73 -> common 17.88); (3) student > teacher everywhere, margin in the delay cells (privileged control_delay latent least trained) -- latent-error decomposition pending (finding/439); (4) d8 (160 ms) 15-20 deg is the remaining weakness; (5) yaw position error 23.7 deg / Reward/yaw -2.3 invisible to the exam; (6) 6/22 DORAEMON updates rejected by SLSQP, invisible in `mode` (finding/437); (7) config is override-only (P5_DELTA + resume-time lb).

**Open leads / next (exp-design's call)**: regrade `sd_r3a` + `sd_inc9998` on the p5 final sidecar with the p5 delta (finding/438, only way to rank the pack vs the robot's policy); decompose the student latent error per dim (finding/439); finding/264 stays open (task-cfg default still (0,3); p5 uses the override); finding/163 test-level xpass only. Other operator-deferred items unchanged (G10 water run, decision/159 결정 3, decision/390 board gate, s31 student, WP11 arm, candidate C/401, finding/378, finding/410, marinelab push, G2 re-run). Security note still open: `~/workspace/.claude/settings.local.json` on the Mac carries an SSH password inside an allow rule.

**Leftovers on the live tree (untouched)**: `eval.py.bak-envdr`, `eval.py.bak-envdr2`, `config.py.bak-260907`, `doraemon.py.bak-260907`, `PLAN.md.bak-20260907c`, `PLAN.md.bak-20260908`, `scratch_find_dead.py`, `scratch_fixtures.txt`, `scripts/isaac_n2_static_map.py`, `tools/check_env_dr_anchor.py`; `/root/p5_report_scratch/` (analysis scripts + engine outputs), `/tmp/albc-testenv`.
