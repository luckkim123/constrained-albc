...
- (2026-09-07, ksm-mac-session) SECOND INSTANCE, one layer up (2026-09-07 05:03). The fix for this finding -- adding HEAD/dirty provenance to sd_exam_generic2.sh -- was applied at ~02:12 WHILE THAT SCRIPT WAS RUNNING. bash reads a script incrementally from disk, so inserting ~140 bytes at line 43 shifted every later offset and the shell never reached its final touch of the DONE marker. Consequences: SD_R3AS31_EXAM_DONE was never created; the redo watcher waiting on it gave up after 30 min so the contaminated cell was NOT re-run; and regrade_chain waited its full 180 min and started 2 h late at 04:31. The exam data itself is intact (5 of 5 summary.json). Marker created by hand, cell discarded and re-run at 05:03 -- and the new provenance line proves the re-run is clean: HEAD=c5680ed dirty=0. So the rule generalises beyond imported modules: a running bash script is also read from disk as it executes. Edit a copy and swap it in, or wait for the chain.
