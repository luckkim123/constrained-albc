...
- verified: 2026-08-04 · keywords: profile, auto-synced
- summary: Workspace profile (auto-synced)

Regenerated from `.omx/profile/` at 2026-08-04T06:37:18.455933. Do not edit; run `omx wiki sync-profile` after profile changes.
...
# Reward/total alone (rule 03 differential diagnosis). Reward/total is
...
# performance_lb (=250, config.py:601) for success, so keep it for the curriculum analysis.
- Train/mean_reward
- Reward/att_rp
- Reward/yaw_vel
- Reward/bias
...
# converge, constraint advantages are noise and IPO misbehaves. Loss/learning_rate and
# Noise/std_min are omitted as redundant. Token names verified against a real event file
# (E-int, 2026-08-04): bare entropy/noise_std/line_search_success/kl never existed as tags;
# the Policy/* and Loss/* forms below are the only real names (G4 gate, dgx-final-scaleup).
- Policy/entropy
...
groups:
  ...
  trpo: [Policy/entropy, Policy/mean_noise_std, Policy/line_search_success, Loss/kl, Policy/surrogate_loss, Grad/actor_step, Grad/sigma_step]
...
roots: index: experiments, data: logs/rsl_rl
levels: index: level1/level2/level3?; data: (flat)
links: train
