...
- verified: 2026-08-04
- summary: Campaign-wide latent R2 census (11 student arms): negative R2 is NOT the campaign norm, the gen-1 extra-obs arm is the best tracker, and the obs76 gen-2 student sits at the bottom while being the only arm that kills envs
...
## Update (2026-08-04T06:43:39.093869)

CORRECTION 2026-08-04 (same day, to the close-out note above): the sentence "ranking student arms by latent R2 does not rank them by control quality" overstated what X1 showed. X1s control null is what the campaigns measured actor sensitivity PREDICTS for a latent move of that size - the R2 swing of +0.169 is only a 6.69% latent-RMSE reduction, worth about 0.057 deg at hard against a 0.10 deg floor. So the census has not been shown to mis-rank; it has been shown to report a metric whose DELTAS are not directly comparable to control floors.

The usable rule: a census R2 delta must be converted to an RMSE change and priced through the level-specific deg-per-unit sensitivity before any control claim is attached to it. Under that conversion most of this censuss inter-arm spread is worth well under the control floor, which is why arm selection should not be driven from this table.
