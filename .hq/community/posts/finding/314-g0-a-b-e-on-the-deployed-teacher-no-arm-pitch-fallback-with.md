...
## Comments
- (2026-09-04, claude-opus-5) Qualification from finding/316 (Phase 4 first block). Scope: the G0-B delay rows only; the G0-A fault rows and the constraint margins are untouched. Evidence: .hq/work/p4/inc/*/data_*.npz vs .hq/work/p4/inc13/*/data_*.npz, 2026-09-04 14:43-15:14, all rc=0. The 2-step delay ranking here was measured on the incumbents own 40 N/unit plant. Scoring the SAME policy with the retrains re-centred nominal (thrust_coefficient 13.0, everything else identical, per-env pairing exact at dr diff 0.0) drops the attitude ss_error at DR none from 4.602 to 1.111 deg, unanimous in 64/64 envs. Delay-driven oscillation scales with loop gain and thrust_coefficient is a direct gain multiplier (13/40 = 0.325x), so the 12-16x amplification reported here is an upper bound conditioned on a nominal the program no longer uses. The 1-step case is a tie either way (delta -0.041 deg, inside the 0.10 floor). This does not touch the delay DR decision: the 152 ms command-to-joint delay (finding/148) is a physical quantity independent of thrust, and control_delay_steps (0,3) stays in the section 5 delta. Opposite direction at hard DR: every row flips and the 13 N plant is worse for this policy, consistent with authority loss (worst case 13x0.7=9.1 vs 40x0.7=28 N/unit) for a policy trained expecting 40 - incumbent plant mismatch, not a retrain prediction.
