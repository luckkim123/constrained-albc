...
ranges — see `constrained_albc/algorithms/student/runner.py`'s pattern). Full mechanics (Beta math, `step()` control flow,
...
### Post-construction override (matches `constrained_albc/algorithms/student/runner.py`'s pattern)
