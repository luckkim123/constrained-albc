...
- `act()` (`constrained_albc/algorithms/encoder/actor_critic_encoder.py:275`): builds `actor_obs = cat[EmpiricalNorm(o_t), z_raw]`,
...
- `constrained_albc/algorithms/encoder/actor_critic_encoder.py` — `act()` / `act_inference()` (no clamp), actor obs assembly
- `constrained_albc/algorithms/encoder/_policy_base.py` — global `log_std`, Gaussian `_update_distribution`
