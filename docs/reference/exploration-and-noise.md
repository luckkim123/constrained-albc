...
`PolicyBase._init_base` (`algorithms/encoder/_policy_base.py:96`):
...
`_update_distribution` (`algorithms/encoder/_policy_base.py:128–130`):
...
(`algorithms/constraint_trpo.py:505–511`):
...
(`algorithms/constraint_trpo.py:486–500`):
...
(`algorithms/constraint_trpo.py:156–175`):
...
`algorithms/constraint_trpo.py:176–178`) for logging the sigma component of the flat step.

**FVP includes `log_std`.** The Fisher-vector product
(`_fisher_vector_product`, `algorithms/constraint_trpo.py:368–379`) is a pure KL Hessian on
...
| Object | `log_std` (`algorithms/encoder/_policy_base.py:96`) | `_OBS_NOISE_STD` (`config.py:259`) |
...
iteration (`algorithms/runners/constraint_encoder_runner.py:311, 349–350`):
...
| std clamp application (log space) | — | `algorithms/constraint_trpo.py:505–511` |
| entropy bonus in surrogate | — | `algorithms/constraint_trpo.py:486–500` |
| param-group split (log_std → TRPO) | — | `algorithms/constraint_trpo.py:156–175` |
| FVP includes log_std | — | `algorithms/constraint_trpo.py:368–379` |
| log_std parameter | — | `algorithms/encoder/_policy_base.py:96` |
| `_update_distribution` (std = exp) | — | `algorithms/encoder/_policy_base.py:128–130` |
| entropy/noise log keys | Policy/entropy, Noise/std_mean, Noise/std_min | `algorithms/runners/constraint_encoder_runner.py:311, 349–350` |
...
The action sample is a raw Gaussian with no clamp (`algorithms/encoder/actor_critic_encoder.py:275–280`
...
log-prob is computed on the **pre-clamp raw sample** (`algorithms/constraint_trpo.py:473`) and
...
- `constrained_albc/algorithms/encoder/_policy_base.py` — `log_std` parameter (`:96`), `_update_distribution` (`:128`)
- `constrained_albc/algorithms/constraint_trpo.py` — std clamp (`:505`), entropy bonus (`:486`), param-group split (`:156`), FVP (`:368`)
- `constrained_albc/envs/main/agents/rsl_rl_ppo_cfg.py` — `init_noise_std`, `entropy_coef(_per_dim)`, `min/max_std`, `min_std_per_dim`
- `constrained_albc/algorithms/runners/constraint_encoder_runner.py` — `Policy/entropy`, `Noise/std_*` logging
