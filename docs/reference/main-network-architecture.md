...
- **Fallback**: old hardcoded `_PRIV_OBS_LOWER/UPPER` in `agents/rsl_rl_ppo_cfg.py`, used only at construct time (still imported by `constrained_albc/algorithms/student/teacher.py`); it had drifted from DR (payload-mass overflow, stale CoG-xy radius) — the reason for the derivation refactor.
...
Algorithm body: `constrained_albc/algorithms/constraint_trpo.py`. Objective (from the module
docstring):
...
`num_constraints` (K) is auto-synced by `constrained_albc/algorithms/runners/constraint_encoder_runner.py` from the
...
- **Architecture**: identical 106D asymmetric input (`cat[o_t, z, p_t]`), hidden
  ...
  (`constrained_albc/algorithms/encoder/_policy_base.py`). cfg `cost_critic_hidden_dims` at `rsl_rl_ppo_cfg.py:181`.
...
placeholder (`rsl_rl_ppo_cfg.py:180`); `constrained_albc/algorithms/runners/constraint_encoder_runner.py` overwrites it from
...
- `constrained_albc/algorithms/encoder/_policy_base.py` — `PolicyBase`, global `log_std`, Gaussian, critic/cost_critic
- `constrained_albc/algorithms/encoder/actor_critic_encoder.py` — encoder, actor, asymmetric critic wiring
- `constrained_albc/algorithms/encoder/actor_critic_asym_constrained.py` — NoEncoder ablation policy
- `constrained_albc/algorithms/constraint_trpo.py` — ConstraintTRPO + IPO
- `constrained_albc/algorithms/runners/constraint_encoder_runner.py` — runner, `num_constraints` auto-sync
