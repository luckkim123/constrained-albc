...
> `marinelab/marinelab/algorithms/doraemon.py`의 17-파라미터 DORAEMON 엔트로피
...
> 반영한다. 레거시 full-DOF 변형(`envs/full_dof/`)은 동일한 17-파라미터
...
| 표면 | 정체 | 관리 주체 | 기본 상태 |
|:---|:---|:---|:---|
| **DORAEMON 물리 DR** | 17개 물리 파라미터(질량·감쇠·부력 형상·해류·payload XY-offset) | 학습되는 Beta 커리큘럼(DORAEMON) | **on** (`enable=True`) |
| **Uniform-only DR** | joint gain/friction/effort, thruster scale | reset마다 고정 uniform 샘플 | on (`randomization` 활성 시) |
...
constrained_albc/envs/main/doraemon.py         # ALBC 결합: _PARAM_DEFS(17개) + re-export shim
...
> **업데이트 2026-07-08**: `payload_cog_offset_xy_u`가 DORAEMON으로 승격됨
> (NDIMS 16 -> 17), main에 병합(`b830043`). 커리큘럼은 nominal `u = 0`에서
> 시작하므로 DORAEMON이 확장하기 전까지 기본 훈련 동작은 변하지 않는다.

### 블록 A — DORAEMON 커리큘럼 관리 (17개, Beta 차원 순서)
...
| 15 | payload_cog_offset_xy_u | `payload_cog_offset_xy_u_range` | (0.0, 1.0) | (0.0, 1.0) | **0.0 (override)** | payload-CoG XY 편심을 위한 정규화된 면적-분위수 u ∈ [0,1]. events가 r = payload_cog_offset_xy_radius * sqrt(u)로 물리 반경에 매핑(sqrt = 면적-균등 보정). 커리큘럼은 u=0(XY offset 없음)에서 시작해 u=1(full r_max)까지 확장. 물리적 r_max(payload_cog_offset_xy_radius=0.08)는 고정 상수이며, 랜덤화되는 것은 u뿐이다. |
| 16 | ocean_current_strength | `ocean_current_strength_range` | (0.0, 1.0) | (0.0, 1.0) \* | **0.0 (override)** | `ocean_current.max_velocity`에 곱하는 스칼라 [0,1]. 커리큘럼 시작 0(무해류), 정책이 쉬운 변형을 숙달하면 확장. |

\* `HardDomainRandomizationCfg`는 `water_density`와 `ocean_current_strength`를
오버라이드하지 **않는다**. 이 둘은 hard == soft.

`NDIMS = 17` (`envs/main/doraemon.py`). `ocean_current_strength`는 r13에서
추가됐고, 그래서 `HardDR` docstring(`config.py:187`)의 낡은 주석이 아직도 "15
parameters"라고 한다 — 그 주석은 틀렸고 실제는 17개다.
...
| name | SOFT | HARD | 의미 |
|:---|:---|:---|:---|
| payload_cog_offset_xy_radius | 0.08 | 0.08 | 물리적 r_max 상수; XY offset 반경은 이제 payload_cog_offset_xy_u(블록 A #15)를 통해 r = r_max * sqrt(u)로 DORAEMON이 관리한다. 이 스칼라는 ceiling만 설정한다. |
...
커리큘럼 상태는 **17개 독립 Beta 분포**이며, 파라미터마다 하나씩 그 물리적
...
디스크에서 `r = r_max * sqrt(u)`로 샘플된다(면적-균등 보정). `u`는 이제
DORAEMON이 관리하는 `payload_cog_offset_xy_u` 차원(블록 A #15)이며, uniform
`torch.rand` 추출이 아니다 — `r_max`만 고정 스칼라다(블록 C).
...
6. **`_PARAM_DEFS` 리터럴 bound는 fallback 전용.** `build_param_specs`가
   ...
   `config.py:187`의 "15 parameters" 주석에도 불구하고 `NDIMS = 17`.
...
| 17개 param 정의 / override | `constrained_albc/envs/main/doraemon.py:41`, `:66` |
