...
> 커널, `RewardManager`가 매 step 합산하는 7개의 가중 항, 그것들을 먹이는
...
한 control step의 reward는 **7개 항의 `dt`-스케일 가중합**이다.
`RewardManager.compute()`(`rewards.py:208-227`)가 합을 만든다; 모든 항은
...
7개 항(`RewardManager._NAMES`, `rewards.py:199`)은 세 개의 **tracking
...
`att_rp`와 `yaw_vel`이 실제로 합산되는 유일한 두 tracking 항이다. 세 번째
tracking 함수 `lin_vel_tracking`은 파일에 존재하지만 `RewardManager`에 **연결돼
있지 않다**(§4.3, §9) — 이 attitude-only env에서 dead code다. 여덟 번째 reward
기여인 `termination_penalty`는 `compute()` 바깥에서 적용되며 7개의
tracked/logged 항에 **속하지 않는다**(§5.5, §9).

---

## 2. 공유 tracking 커널 — `_exp_quad_saturating`

세 tracking 함수 모두 하나의 커널 `_exp_quad_saturating(err_sq, err_norm,
term)`(`rewards.py:97-117`)을 호출한다. 이것은 양의 Gaussian bump에 최대 세 가지
...
(`rewards.py:97-117`에 `term.k` 참조가 없다); 나중에 `compute()`가 곱한다(§6).
`TrackingTermCfg` docstring의 공식 `r = k * (exp(...) - ...)`
(`rewards.py:49`)는 함수가 실제로는 적용하지 않는 `k`를 접어 넣은 것이며 — 그
...
`tests/test_rewards.py:113,124,139`로 고정)이며; 그 slope
$-\tfrac{e}{\sigma^2}\exp(-e^2/2\sigma^2)$는 $|e|=\sigma$에서 최대이고 $e=0$
근처에서 0으로 붕괴한다. 따라서 exp와 quadratic gradient 모두 **`e -> 0`에서
소멸**하며, 이를 모듈 docstring은 "SS-error dead zone"(`rewards.py:14-17`)이라
...
### 2.2 `lin_ratio` dead-zone 함정과 docstring 모순

`lin_ratio`는 기본값이 `0.0`(`rewards.py:59`, 주석 `# linear penalty ratio
(disabled: caused dead zone)`)이고 **어디에서도 nonzero로 설정되지 않는다** —
세 `TrackingTermCfg` 생성(`rewards.py:70,72,73`)도, config
override(`config.py:423`)도 이를 넘기지 않는다. 그래서 shipped config에서 linear
penalty는 세 tracking 항 모두에서 무력하다.

이것은 `rewards.py` 내부의 진짜 자기모순이다: 모듈 docstring은 linear 항을 *"SS
error tolerance를 위한 결정적 fix … |e|에 대한 일정한 하방 force로, 정책이 SS
error를 0까지 계속 줄이도록 보장"*(`rewards.py:14-17`)이라 서술하는 반면,
필드 레벨 주석(`rewards.py:59`)은 그것이 dead zone을 *유발했기 때문에*
비활성화됐다고 말한다. 두 진술 모두 파일에 있고; 이를 해소하는 주석은 없다. 배포된
상태로는 docstring이 경고하는 SS-error dead zone이 존재하며 — `yaw_vel`에서만 그
tanh penalty(§2.3)로 완화되고, `att_rp`에서는 전혀 완화되지 않는다.

### 2.3 tanh vs arctan: 코드가 아니라 관례로 강제됨

두 saturating 블록은 **독립적인 `if` 문**(`rewards.py:111`과 `:113`)이다; 두 계수
모두 양수라면 둘 다 fire되는 것을 막는 것은 없으며 — 그러면 additively 쌓일
것이다. docstring의 *"tanh/arctan 중 한 번에 하나만"*(`rewards.py:51`)은
config-authoring 관례이지 코드 불변식이 아니다. shipped config에서는 `tanh_coef`만
nonzero이고, `yaw_vel`에서만 그렇다(`config.py:423`); `arctan_coef`는 결코 설정되지
않는다(§3, §4).
...
"smoothness", "bias", "joint1_center"]`(`rewards.py:199`) — 7개 항이며,
`compute()`의 `terms` 리스트(`rewards.py:213-221`)에서 1:1로 조립된다.
...
`ALBCRewardCfg`의 12개 필드 중 **정확히 두 개**만 shipped config가 건드린다
(`config.py:422-430`): `yaw_vel`(tanh penalty를 얻음)과 `k_bias`(`-2.0`에서 켜짐).
...
*실질적* 변경은 tanh 항을 켜는 것뿐이다. `att_rp`, `lin_vel`, `lin_ratio`,
...
30 s episode(`episode_length_s=30.0`, `config.py:334`; 1500 step) 동안 `att_rp`만
...
`att_rp_tracking`(`rewards.py:128-138`)은 두 개의 error aggregate를 만들어 커널에
넘긴다:
...
`att_roll_weight = 1.5`(`rewards.py:71`)로 up-weight되므로, roll error가 단위당
pitch보다 더 비싸다(`test_att_rp_roll_weighted_more_than_pitch`,
`tests/test_rewards.py:131`로 고정). shipped config에서 `att_rp`에는 saturating도
...
error가 penalize되기 전에 up-weight된다(주석 `rewards.py:71`: "weak TAM
actuation: 0.007m vs pitch 0.145m").

### 4.2 `yaw_vel_tracking` — scalar, shipped config의 유일한 saturating 항

`yaw_vel_tracking`(`rewards.py:141-144`)은 scalar yaw *rate* error를 추적한다(yaw
각도가 아님):
...
기본값(`rewards.py:73`)에는 saturating 항이 없지만, shipped
config(`config.py:423`)가 `yaw_vel`을
...
### 4.3 `lin_vel_tracking` — 정의되어 있으나 구조적으로 DEAD

`lin_vel_tracking`(`rewards.py:121-125`)은 진짜 Euclidean norm(`err_norm =
sqrt(err_sq)`, 정확히 0에서 `sqrt`를 통한 NaN gradient를 피하려고 `1e-12`로
clamp)을 쓰는 유일한 tracking 함수다. `# UNUSED in attitude_only (kept for cfg
compatibility; not in RewardManager).`(`rewards.py:120`)로 표시돼 있으며
`_NAMES`(`rewards.py:199`)와 `terms` 리스트(`rewards.py:213-221`) **양쪽에서
빠져 있다**. `ALBCRewardCfg.lin_vel` 필드(`rewards.py:72`)는 레거시 full-DOF
env와의 dataclass/config shape 호환을 위해서만 존재한다.

이것은 단지 dormant-but-safe가 아니라 — main에서 live `ALBCEnv`에 대해 호출되면
**crash**할 것이다: `env._lin_vel_err`는 `envs/main/albc_env.py` 어디에서도
할당되지 않으므로, 호출은 `AttributeError`를 raise한다. repo 내 유일한 caller는
`SimpleNamespace` 대역에 `_lin_vel_err`를 공급하는 unit test
`test_lin_vel_tracking_peaks_at_zero_error`(`tests/test_rewards.py:113`)다. 형제
`envs/full_dof/mdp/rewards.py`(별개의 hand-fork 파일)에서는 `lin_vel_tracking`이
그 변형의 `RewardManager`에 **연결돼 있다** — 즉 dead-code 상태는 main에 국한된다.

`rewards.py:6,20`의 모듈 docstring은 여전히 `r_lin`을 합산 항으로 나열하지만; 이는
stale하다(§9).
...
항과 동일하게 `dt`-스케일된다(§6) — 7개 항 전부에 대해 스케일링 지점은 하나다.

### 5.1 `joint_torque` — post-clamp applied torque

`joint_torque`(`rewards.py:147-149`): `mean(applied_torque[albc_joints]²)`.
...
주석 `rewards.py:148`: "post-clamp applied torque"). 이것이 raw action magnitude를
...
`thruster_energy`(`rewards.py:152-154`): `mean(env._actions[:, 2:]²)`. action
레이아웃은 `[2D arm delta, 6D thruster]`(`action-pipeline.md` §2)이므로, `[:, 2:]`
슬라이싱은 6개 thruster 차원(인덱스 2-7)을 취하고 **2개 arm 차원은 제외한다**.
`test_thruster_energy_uses_thruster_action_slice`(`tests/test_rewards.py:154`)로
...
`action_smoothness`(`rewards.py:157-161`): `mean(da²) + mean(d2a²)`이며 여기서
...
(`test_action_smoothness_zero_when_constant`, `tests/test_rewards.py:161`). 세
...
`bias_ema_penalty`(`rewards.py:164-172`): 3-channel `env._bias_ema`(roll, pitch,
yaw-rate)에 대한 `Σᵢ wᵢ · bias_emaᵢ²`이며, per-axis 가중치
`env._reward_manager._bias_w`는 `cfg.bias_weights`에서 한 번 preallocate된다
(`rewards.py:206`; 기본값 `(1.5, 1.0, 1.0)`, `rewards.py:91`). per-step tracking
...
shipped config에서는 `k_bias=-2.0`으로 **ON**이다(`config.py:429`).

**env-side 결합(two-file 함정).** `albc_env.py:1042-1052`의 EMA 버퍼 업데이트
자체가 `if self.cfg.reward.k_bias != 0.0:`로 gating되므로, `k_bias`를 on/off하면
서로 다른 두 파일에서 결합된 두 가지가 뒤집힌다: (a) reward 항의 가중치가 nonzero가
되고(`rewards.py:219`), (b) EMA 업데이트 루프 `bias_ema = α·bias_ema +
...
### 5.5 `joint1_centering_penalty` — 기본 off, angle-wrapped

`joint1_centering_penalty`(`rewards.py:175-190`): `wrap = atan2(sin θ₁, cos θ₁)`인
`wrap(θ₁)²`이며 joint1만 읽는다(`env._albc_joint_ids[0]`, `rewards.py:188`).
joint1은 PhysX position limit이 없는 연속-회전 모터로, 순수 delta-integrator로
구동된다(`action-pipeline.md` §4.1); 아무것도 그것을 nominal로 되당기지 않으므로,
sim-to-real micro-bias가 실로봇에서 이를 monotonically drift시킬 것이다. 이 항이
누락된 restoring gradient를 공급한다. wrap은 한 바퀴 전체($\theta_1 = 2\pi$,
물리적으로 0과 동일)가 큰 error로 penalize되지 **않도록** 보장한다 — 진짜 각도
거리만 penalize된다(`test_joint1_centering_wraps_full_revolution`,
`tests/test_rewards.py:222`).

기본값으로 **OFF**이며(`k_joint1_center=0.0`, `rewards.py:83`, 주석: "byte-identical
to runs without this term") shipped config가 결코 override하지 않는다. 여전히 **매
step 실행되고**(`atan2/sin/cos`가 돎) 정확히 `0.0`을 기여한다는 점에 유의 — 따라서
`Reward/joint1_center`는 항상 `0.0`을 log하는데, 이는 "측정된 0"이 아니라
"disabled"로 읽힌다. constraint 쪽 joint1 anti-drift 메커니즘
(`joint1_constraint_arm`, `main-network-architecture.md` §5.1)은 이 reward 쪽
항과는 별개이며 관례상 상호 배타적인 lever다.
...
호출된다(`albc_env.py:1054-1058`). `compute()`(`rewards.py:222-225`) 내부:
...
한다(예: `:101`). `sim.dt = 0.005`(`config.py:356`)와
`decimation = 4`(`config.py:335`)에서 `step_dt = 0.02 s`(50 Hz)다. 그래서 보고되는
`k` 값은 **초당**이고; 실효 per-step은 `k * 0.02`다(§3.2). 모든 항 — tracking과
penalty 모두 — 이 단일 지점에서 스케일된다; 유일하게 제외되는 reward 기여는
`compute()` 반환 *이후* 더해지는 `termination_penalty`(`albc_env.py:1061-1062`,
`* dt` 없음)이며, 이것이 7개 tracked 항에 속하지 않는 이유다.
...
documentation-vs-code 불일치다 — 실행되는 코드 경로는 내부적으로 일관된다(7
name, 7 term, 7 episode-sum key).
...
### 10.1 테스트 불변식 — `tests/test_rewards.py` (17개 테스트)

스위트는 repo-root `tests/test_rewards.py`에 있으며(`constrained_albc/` 패키지
**아래가 아님**) 257 줄이다. `importlib`로 `rewards.py`를 직접 import하고
...
7-term 합, `RewardManager.reset()`, tanh/arctan 분기(shipped `yaw_vel` tanh
override는 unit 레벨에서 untested), `lin_ratio > 0`, env 레벨 `k_bias != 0`
gating을 구동하는 테스트가 없다. `bias_w`/`bias_ema` fixture는 6-D 벡터
(`tests/test_rewards.py:176-177`)를 쓰는데 — 이는 test-only shape 선택이지 6-D
...
`_episode_sums[name]`에 누적한다(`rewards.py:224-225`). reset 시
`RewardManager.reset(env_ids)`(`rewards.py:229-234`)는
...
`_collect_episode_metrics`(`albc_env.py:1079-1098`)가 각각을 log key로 바꾼다:
...
`Reward/joint1_center`(`_NAMES`와 일치)에 파생 `Reward/total`이다. wandb curve를
...
- **고정 분모.** `max_episode_length_s`는 `cfg.episode_length_s`(base
  ...
  log되므로(`albc_env.py:1090`) single-env 극단 trajectory를 가중할 수 있다.
...
- `constrained_albc/envs/main/mdp/rewards.py` — `TrackingTermCfg`, `ALBCRewardCfg`, `_exp_quad_saturating`, 7개 term 함수, `RewardManager`(`compute`/`reset`)
- `constrained_albc/envs/main/config.py` — `reward` override(`:422-430`), TAM(`:77-84`), command range(`:410-417`), `integral_gated`(`:347`), sim/decimation/episode length(`:334-356`), `doraemon`/`performance_lb`(`:466-479`)
...
- `tests/test_rewards.py` — 17개 whitebox sign/roll-weight/wrap pin(repo-root `tests/`, 패키지 아래가 아님)
