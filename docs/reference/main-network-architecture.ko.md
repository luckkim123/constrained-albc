...
아니다(`encoder_obs_normalization=False`, `:136`): cfg가 27-element `_PRIV_OBS_LOWER` /
`_PRIV_OBS_UPPER`를 넘기고, encoder가 `(2*p_t - (U+L)) / (U-L)`를 계산한다.
running statistics 없이 결정론적이라 `z` drift / KL spike를 피한다.
...
ROLLOUT -> reward GAE + constraint별 cost GAE (K constraints, cost storage shape (T,N,K))
  |
update():
  - IPO adaptive thresholds:  d_k^i = max(d_k, J_Ck + alpha*d_k)   # 위반 시 threshold 상향, -log 폭주 방지
