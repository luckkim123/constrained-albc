...
| E-int | integration retrain (anchor + B0c band + fault-DR); **LAUNCHED 2026-07-27 16:09 (human-approved) as `trpo_eint_s30_260727_160913`** in group `teacher_baseline_buoyfix` (user kept the existing group over a new `teacher_final`); proposal `next-20260727-155500` (independent review FINAL APPROVE after 3 rounds; verdict rule pre-registered and proven exhaustive by exclusion logic — H2 / ADOPT-WITH-CAVEAT / H1, m4-dead applied per level with the worst level deciding), code commit `39e819a`, ~5.5 h at 4.0 s/iter; both knobs verified in the run's `params/env.yaml` (band `(0.85, 1.15)`, `fault.enable: true`, `use_privileged_fault_obs: false`); D-a and D-b both adopted (user); whether E-ftc1 precedes it pends the user's re-judgment of E-ftc1 (the user declined an "8k extension" reading 2026-07-27; the as-designed probe is 5k-fixed, see 12.2 E-ftc1) | workstation, ~5 h | becomes the final teacher -> C4 |
