...
| HydroRC | hydro DR nominals recentered to the Stonefish-measured effective values (marinelab `exp/hydro-recenter` 016d1b1, parent f45d612 = E-int marinelab state; linear_damping (2,2,1.5,0.3,0.3,0.15)->(0.5,0.5,1.6,0.03,0.03,0.002), quadratic_damping (39,39,15,1,1,0.5)->(8,8,22.7,1.2,1.2,0.011), added_mass heave 1.0->8.0 cap-bound). Deployment-driven probe (proposal `next-20260727-174905`, buoyanchor lineage, reviewed+revised 2026-07-27): discriminates Lane-2 (plant damping mis-center; measured yaw 45-100x / pitch-linear ~0 / translational 3-8x below the analytical nominal, deployment plant OUTSIDE the trained DR box) vs Lane-1 (integration seams) as the cause of the Stonefish 50 Hz attitude-hold limit cycle (pitch RMS 6.1 deg, yaw-rate RMS 42 deg/s). **LAUNCHED 2026-07-28 01:31 KST (user-approved) as `trpo_hydrorc_s30_260728_013136`** -- single variable vs E-int (same seed 30, same machine GPU0, constrained-albc code identical). Readout ORDER (pre-registered): Isaac paired same-seed non-regression gate FIRST (none-level roll/pitch/yaw ss_error + roll n_gt20 as paired deltas vs E-int, 16.8% provisional same-machine bound; any breach -> revise nominals, do NOT proceed) -> only then the Stonefish diagnostic on the Stonefish machine (H1: pitch RMS <=3.0 AND yaw-rate RMS <=20 deg/s AND m0/m3 duty <30%; H2: within +-30% of baseline; middle zone -> keep recenter + continue Lane-1 probes). NOTE: probes the deployment-facing lineage only -- the E-int final-teacher verdict is UNCHANGED by this run | workstation GPU0, ~5.5 h (in flight) | Isaac gate -> Stonefish 50 Hz attitude-hold vs buoyfix baseline |
