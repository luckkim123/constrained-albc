---
title: "Exactly one constraint is binding (`thruster_util`) and its margin behaves like "
...
# Exactly one constraint is binding (`thruster_util`) and its margin behaves like 

Exactly one constraint is binding (`thruster_util`) and its margin behaves like the narrow-width reference, not like extend8k — the width-tracking half of the decoupling above.

[EVIDENCE: engine `[TIER 2] Constraints`, all 10 rows, three runs]
[CONFIDENCE: HIGH]
