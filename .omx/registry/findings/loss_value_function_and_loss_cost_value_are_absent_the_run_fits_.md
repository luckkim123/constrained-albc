---
title: "`Loss/value_function` and `Loss/cost_value` are absent: the run fits no critic."
...
# `Loss/value_function` and `Loss/cost_value` are absent: the run fits no critic.

`Loss/value_function` and `Loss/cost_value` are absent: the run fits no critic.

[EVIDENCE: 0 tags under the `Loss/` prefix]
[CONFIDENCE: HIGH]
...
[EVIDENCE: 0 tags under the `Loss/` prefix in the dumped census]
[CONFIDENCE: HIGH]
