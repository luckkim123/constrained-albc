---
title: "Encoder TB-health is unchanged: Encoder/z_std 0.40 -> 0.38, z range symmetric an"
tags: ["auto-captured", "trpo_b0cmaxthrust_s30_260724_024326"]
created: 2026-07-27T06:42:48.885806
updated: 2026-07-27T06:42:48.885806
sources: ["experiments/rsl_rl/albc_trpo_teacher/teacher_baseline_buoyfix/trpo_b0cmaxthrust_s30_260724_024326/analysis/diagnose-20260727-151917/report.md"]
links: []
category: session-log
confidence: low
schemaVersion: 1
qualityScore: 100
qualityReasons: []
---

# Encoder TB-health is unchanged: Encoder/z_std 0.40 -> 0.38, z range symmetric an

Encoder TB-health is unchanged: Encoder/z_std 0.40 -> 0.38, z range symmetric and unsaturated (Encoder/z_min / Encoder/z_max: [-0.72, 0.73] -> [-0.73, 0.71]), z_mean near 0 in both — no collapse or saturation signature.

[EVIDENCE: engine TIER 1 z rows, both runs]
[CONFIDENCE: HIGH]

source report: experiments/rsl_rl/albc_trpo_teacher/teacher_baseline_buoyfix/trpo_b0cmaxthrust_s30_260724_024326/analysis/diagnose-20260727-151917/report.md
