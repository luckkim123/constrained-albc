---
title: "The loss reproduces on roll and pitch separately, so it is a whole-attitude effe"
...
# The loss reproduces on roll and pitch separately, so it is a whole-attitude effe

The loss reproduces on roll and pitch separately, so it is a whole-attitude effect rather than one axis dragging the norm.

[EVIDENCE: `compare.py paired`, per-axis `ss_error` deltas (deg); E-ftc1 / Arm A]
[CONFIDENCE: HIGH]
