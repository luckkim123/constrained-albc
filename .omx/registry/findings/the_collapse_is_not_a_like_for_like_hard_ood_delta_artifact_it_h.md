---
title: "The collapse is not a like-for-like hard/ood delta artifact — it holds on the ON"
...
# The collapse is not a like-for-like hard/ood delta artifact — it holds on the ON

The collapse is not a like-for-like hard/ood delta artifact — it holds on the ONLY strictly-fair cross-run level (none, zero DR, nominal plant for both runs): max/median 7.05x -> 4.26x, top-6 share 30% -> 18%. At hard/ood it is a REGIME change (crossing the 10x line, ~half -> ~quarter concentration), not a marginal move, so it survives the DORAEMON run-relative-DR caveat below.

[EVIDENCE: none-level rows of the table above, per-env roll ss code-exec]
[CONFIDENCE: HIGH]
...
## Update (2026-07-16T07:19:04.274555)
