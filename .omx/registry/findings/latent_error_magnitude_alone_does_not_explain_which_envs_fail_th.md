---
title: "Latent error magnitude alone does not explain which envs fail — the two dead env"
tags: ["auto-captured", "trpo_sdobs76_c3_gruselect_s30_260804_124951"]
created: 2026-08-04T04:31:12.085504
updated: 2026-08-04T04:31:12.085504
sources: ["experiments/rsl_rl/albc_trpo_student/student_distill_obs76/trpo_sdobs76_c3_gruselect_s30_260804_124951/analysis/diagnose-20260804-132500/report.md"]
links: []
category: session-log
confidence: low
schemaVersion: 1
qualityScore: 100
qualityReasons: []
---

# Latent error magnitude alone does not explain which envs fail — the two dead env

Latent error magnitude alone does not explain which envs fail — the two dead envs are not the two worst-latent envs.

[EVIDENCE: per-env latent RMSE at hard, median 0.2566 / max 0.4984; dead env 19 = 0.2680 (rank 26/64), dead env 39 = 0.3637 (rank 9/64)]
[CONFIDENCE: MED]

source report: experiments/rsl_rl/albc_trpo_student/student_distill_obs76/trpo_sdobs76_c3_gruselect_s30_260804_124951/analysis/diagnose-20260804-132500/report.md
