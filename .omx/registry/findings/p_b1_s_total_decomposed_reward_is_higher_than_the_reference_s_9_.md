---
title: "P-B1's total decomposed reward is higher than the reference's (9.16 vs 7.39), dr"
tags: ["auto-captured", "trpo_biasema_260715_142543"]
created: 2026-07-15T10:45:08.430019
updated: 2026-07-15T10:45:08.430019
sources: ["experiments/rsl_rl/albc_trpo_teacher/teacher_baseline_posttam/trpo_biasema_260715_142543/analysis/diagnose-20260715-193800/report.md"]
links: []
category: session-log
confidence: low
schemaVersion: 1
qualityScore: 100
qualityReasons: []
---

# P-B1's total decomposed reward is higher than the reference's (9.16 vs 7.39), dr

P-B1's total decomposed reward is higher than the reference's (9.16 vs 7.39), driven mainly by a higher `att_rp` term (attitude-tracking reward component) — consistent with the sharply lower roll/pitch tracking error at `none`.

[EVIDENCE: engine deep output, both runs]
[CONFIDENCE: HIGH]

source report: experiments/rsl_rl/albc_trpo_teacher/teacher_baseline_posttam/trpo_biasema_260715_142543/analysis/diagnose-20260715-193800/report.md
