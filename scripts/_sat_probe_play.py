...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    # reset and roll out  [SAT-PROBE]
    import numpy as _np
    core = env.unwrapped
    _raw, _clamped, _state = [], [], []
    _STEPS = 300
    obs = env.get_observations()
    _t = 0
    while simulation_app.is_running() and _t < _STEPS:
        with torch.inference_mode():
            actions = policy(obs)
            _raw.append(actions[:, 2:].detach().cpu().numpy().copy())
            obs, _, dones, _ = env.step(actions)
            _clamped.append(core._actions[:, 2:].detach().cpu().numpy().copy())
            if getattr(core, '_thruster', None) is not None:
                _state.append(core._thruster.state.detach().cpu().numpy().copy())
            if hasattr(policy_nn, 'reset'):
                policy_nn.reset(dones)
        _t += 1

    _raw = _np.concatenate(_raw, 0)
    _clamped = _np.concatenate(_clamped, 0)
    _state = _np.concatenate(_state, 0) if _state else None
    def _rep(name, arr):
        print('\n===== %s (n=%d) =====' % (name, len(arr)), flush=True)
        print('  ch |    min    mean     max     std | sat|.|>=0.99')
        for i in range(arr.shape[1]):
            v = arr[:, i]; sat = (_np.abs(v) >= 0.99).mean()*100
            print('  m%d | %+.3f %+.3f %+.3f  %.3f | %5.1f%%' % (i, v.min(), v.mean(), v.max(), v.std(), sat), flush=True)
    print('\n' + '='*60, flush=True)
    print('SIM ROLLOUT THRUSTER SATURATION PROBE  steps=%d envs=%d' % (_STEPS, core.num_envs), flush=True)
    print('='*60, flush=True)
    _rep('RAW policy output action[2:8] (clamp before, det mean)', _raw)
    _rep('CLAMPED env._actions[2:8] (physics input)', _clamped)
    if _state is not None:
        _rep('FILTER _thruster.state (= obs[14:20] echo)', _state)
