...
"""Analyze latent_log_*.npz output from diagnose_student_latent.py.

Loads latent logs from one or more diagnostic runs and produces:
    - per-dim MSE + per-dim env-var table across DR levels
    - ratio l_hat_envvar / l_true_envvar (tells us if student is over- or
      under-varying compared to ground truth)
    - histogram + scatter plots for latent error
    - per-DR, per-dim summary printed to stdout and saved to JSON

Usage:
    python3 scripts/analysis/analyze_student_latent.py <diag_dir_1> [<diag_dir_2> ...]
"""
...
def load_level(diag_dir: Path, level: str) -> tuple[np.ndarray, np.ndarray] | None:
    f = diag_dir / f"latent_log_{level}.npz"
    if not f.exists():
        return None
    z = np.load(f)
...
def summarize(l_hat: np.ndarray, l_true: np.ndarray) -> dict:
    err = l_hat - l_true
    per_dim_mse = (err ** 2).mean(axis=(0, 1))           # (D,)
    per_dim_envvar_true = l_true.var(axis=1).mean(axis=0)  # (D,)
    per_dim_envvar_hat = l_hat.var(axis=1).mean(axis=0)    # (D,)
    per_dim_tvar_true = l_true.var(axis=0).mean(axis=0)   # (D,)
    per_dim_tvar_hat = l_hat.var(axis=0).mean(axis=0)     # (D,)
    return {
...
def main(argv: list[str]) -> int:
    if len(argv) < 1:
        print("Usage: analyze_student_latent.py <diag_dir> [<diag_dir> ...]")
        return 1
    diag_dirs = [Path(p) for p in argv]
    ...
    if len(diag_dirs) >= 1:
        ...
        plot_comparison(diag_dirs, out_png)
