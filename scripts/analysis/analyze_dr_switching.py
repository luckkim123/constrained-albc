"""Post-hoc analysis of eval_dr_switching output (cascade-PID, target xyz=0 rpy=0).

Compares runs that faced identical DR sequences. Reads summary_switching.json
and per-level eval_<level>.npz from each run's eval_dr_switching/ dir.

Usage:
    python3 analyze_dr_switching.py <run_A> <run_B> [--labels LA LB]
"""
...
def _all_post_switch(run: dict, lvl: str, key: str) -> np.ndarray:
    per = run["summary"]["metrics"][lvl]["per_seg"]
    return np.concatenate([np.array(p[key]) for p in per[1:]])  # skip seg 0


def print_aggregate(runs: dict[str, dict], levels: list[str]) -> None:
    print(f"\n{'=' * 100}")
    ...
    for lvl in levels:
        for name, run in runs.items():
            pos_peak = _all_post_switch(run, lvl, "pos_drift_peak")
            pos_ss = _all_post_switch(run, lvl, "pos_drift_ss")
            rp = _all_post_switch(run, lvl, "peak_roll_deg")
            pp = _all_post_switch(run, lvl, "peak_pitch_deg")
            yp = _all_post_switch(run, lvl, "peak_yaw_deg")
            rs = _all_post_switch(run, lvl, "ss_roll_deg")
            ps = _all_post_switch(run, lvl, "ss_pitch_deg")
            ys = _all_post_switch(run, lvl, "ss_yaw_deg")
...
def heavy_tail_table(runs: dict[str, dict], levels: list[str]) -> None:
    print(f"\n{'=' * 100}")
    ...
    for lvl in levels:
        for name, run in runs.items():
            vals = _all_post_switch(run, lvl, "pos_drift_peak")
    ...
    for lvl in levels:
        for name, run in runs.items():
            for axk, axn in [("peak_roll_deg", "roll"), ("peak_pitch_deg", "pitch"), ("peak_yaw_deg", "yaw")]:
                v = _all_post_switch(run, lvl, axk)
...
def divergence_table(runs: dict[str, dict], levels: list[str]) -> None:
...
def per_seg_table(runs: dict[str, dict], levels: list[str]) -> None:
    for lvl in levels:
...
def main():
    ...
    parser.add_argument("runs", nargs="+", help="Run dirs or eval_dr_switching dirs")
    parser.add_argument("--labels", nargs="+", default=None)
    parser.add_argument("--levels", nargs="+", default=DR_LEVELS)
    args = parser.parse_args()
    labels = args.labels or [os.path.basename(r.rstrip("/")) for r in args.runs]
    runs = {labels[i]: load_run(args.runs[i]) for i in range(len(args.runs))}

    print_aggregate(runs, args.levels)
    heavy_tail_table(runs, args.levels)
    divergence_table(runs, args.levels)
    per_seg_table(runs, args.levels)
