"""Unified per-env summary for eval_dr_fulldof outputs.

Reads eval_{none,soft,medium,hard}.npz files from a run's eval_dr/ directory
and produces:
  * enhanced_summary.json with all per-axis per-env metrics
  * summary_att.png / summary_lin_vel.png / summary_yaw.png with a unified
    3x2 panel layout:
...
All metrics are per-env: we compute the metric for each of the 64 envs,
then average. This contrasts with the stock eval_dr_fulldof.py which
computes metrics on an ensemble-mean trajectory (envelope smoothing).

Usage
-----
    ./isaaclab.sh -p scripts/analysis/recompute_eval_summary.py <run_dir>
    ./isaaclab.sh -p scripts/analysis/recompute_eval_summary.py <r1>,<r2>  --plot=/tmp/cmp.png
"""
...
DR_COLORS = {"none": "tab:blue", "soft": "tab:green", "medium": "tab:orange", "hard": "tab:red"}

AX_LIN = [("lin_vel_x", "target_vx", "vx"),
          ("lin_vel_y", "target_vy", "vy"),
          ("lin_vel_z", "target_vz", "vz")]
AX_YAW = ("yaw_rate", "target_yaw_rate", "yaw")
AX_ATT = [("actual_roll_deg",  "target_roll_deg",  "roll"),
          ("actual_pitch_deg", "target_pitch_deg", "pitch")]

AXIS_PALETTE = {"vx": "tab:blue", "vy": "tab:orange", "vz": "tab:green",
                "roll": "tab:blue", "pitch": "tab:orange"}


# ---------------- Segment detection ----------------

def find_segments(data: dict) -> list[tuple[int, int]]:
    n = len(data["time"])
...
def classify_segment(data: dict, s: int) -> str:
    if s == 0:
        return "init"
    eps = 1e-6
...
def is_target_zero(data: dict, s: int) -> bool:
    eps = 1e-6
...
def per_env_peak_metrics(actual: np.ndarray, alive: np.ndarray,
...
def per_env_rise_time(actual: np.ndarray, alive: np.ndarray,
                      prev_tgt: float, cur_tgt: float,
...
def per_env_ss_stats(actual: np.ndarray, alive: np.ndarray, cur_tgt: float
...
def compute_enhanced_metrics(npz_path: str, peak_window_sec: float = 2.0) -> dict:
    ...
    for s, e in segments:
        kind = classify_segment(data, s)
        if kind in ("init", "zero", "mixed"): continue
        if is_target_zero(data, s):           continue
        ...
        for act_key, tgt_key, name in ax_list:
            ...
            m = per_env_peak_metrics(seg_actual, alive_seg, prev, cur, peak_window)
            # Rise time (mean + std across envs)
            rt_mean, rt_std = per_env_rise_time(seg_actual, alive_seg, prev, cur, dt)
            m["rise_time"]     = rt_mean
            m["rise_time_std"] = rt_std
            # SS error and jitter (means + stds across envs)
            ss_err, ss_err_std, ss_jit, ss_jit_std = per_env_ss_stats(seg_actual, alive_seg, cur)
            m["ss_error"]      = ss_err
...
def process_run(run_dir: str) -> dict:
    ...
    for level in DR_LEVELS:
        ...
        result[level] = compute_enhanced_metrics(path)
    return result
...
def _level_xlabels():
    return [f"{lvl}\n(DR {int(DR_SCALE[lvl]*100)}%)" for lvl in DR_LEVELS]
...
def _bars_single(ax, values: list[float], ylabel: str, title: str, *,
    ...
    xs = np.arange(len(DR_LEVELS))
    ax.bar(xs, values, color=[DR_COLORS[lv] for lv in DR_LEVELS],
...
def _plot_unified_grouped(run_dir: str, metrics: dict, *,
                          ...
                          ss_source: str = "per_axis") -> str:
    """Generic 3x2 summary plot for a group with multiple axes (att, lin_vel)."""
    surv = [metrics.get(lv, {}).get("survival_pct", float("nan")) for lv in DR_LEVELS]
...
def make_summary_att(run_dir: str, metrics: dict) -> str:
...
def make_summary_lin_vel(run_dir: str, metrics: dict) -> str:
    return _plot_unified_grouped(
        run_dir, metrics,
...
def make_summary_yaw(run_dir: str, metrics: dict) -> str:
...
def print_run_summary(run_name: str, metrics: dict) -> None:
    print(f"\n{'='*120}")
...
def write_run_json(run_dir: str, metrics: dict) -> None:
...
def multirun_comparison_plot(runs: dict, output_path: str) -> None:
...
def process_and_write(run_dir: str) -> dict:
    print(f"\nProcessing: {run_dir}")
    metrics = process_run(run_dir)
    print_run_summary(os.path.basename(run_dir), metrics)
    write_run_json(run_dir, metrics)
    if metrics:
        print(f"  Saved {make_summary_att(run_dir, metrics)}")
        print(f"  Saved {make_summary_lin_vel(run_dir, metrics)}")
        print(f"  Saved {make_summary_yaw(run_dir, metrics)}")
    return metrics


def main():
    if len(sys.argv) < 2:
        print("Usage: recompute_eval_summary.py <run_dir>[,<r2>...]  [--plot=/path.png]")
        sys.exit(1)
    run_dirs = [d.strip().rstrip("/") for d in sys.argv[1].split(",")]
    plot_path = None
    for a in sys.argv[2:]:
        if a.startswith("--plot="): plot_path = a.split("=", 1)[1]

    runs = {}
    for rd in run_dirs:
        metrics = process_and_write(rd)
        runs[os.path.basename(rd)] = metrics

    if plot_path and len(runs) > 1:
        multirun_comparison_plot(runs, plot_path)
